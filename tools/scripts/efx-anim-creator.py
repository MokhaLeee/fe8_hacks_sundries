...
def main(args):
    ...
    with open(arguments.input, 'r') as f:
        for line in f.readlines():
            ...
            frames.append(FrameEntry(line))

    if arguments.list_files:
        for frame in frames:
            for filename in frame.list_data_files(arguments.input):
                print(filename, end = ' ')
        ...
        sys.stdout.writelines('\nIMGs:\n')
        for frame in frames:
            sys.stdout.writelines('POIN IMG_{0}\n'.format(os.path.splitext(frame.fPath)[0]))
        ...
        for frame in frames:
            sys.stdout.writelines(frame.gen_event_img_insert(arguments.input))
