...
def lz77_decomp_data(rom, offset):
...
def lz77_compress(src):
    ...
    src_size = len(src)
    if src_size <= 0:
        raise ValueError("源数据为空，无法压缩")

    worst_case_dest_size = 4 + src_size + ((src_size + 7) // 8)
    
    worst_case_dest_size = (worst_case_dest_size + 3) & ~3

    dest = [0] * worst_case_dest_size
    ...
    while True:
        ...
        for i in range(8):
            ...
            while block_distance <= src_pos and block_distance <= 0x1000:
                ...
                while (block_size < 18 and
                       src_pos + block_size < src_size and
            ...
            if best_block_size >= 3:
                ...
                dest[dest_pos] = (best_block_size << 4) | (best_block_distance >> 8)
                dest_pos += 1
                dest[dest_pos] = best_block_distance & 0xFF
                dest_pos += 1
            else:
                dest[dest_pos] = src[src_pos]
                dest_pos += 1
